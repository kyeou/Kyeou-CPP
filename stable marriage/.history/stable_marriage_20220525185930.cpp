...
bool check_unstable(int currManID, int currWomanID) {
        ...
        for (int i = 0; i < N; i++) {
            if (currManID == womens_desire[currWomanID - 1][i]) {
                pref_of_currMan = i;
            }
        }

        for (int i = 0; i < N; i++) {
            if (currWomanID == mens_desire[currManID - 1][i]) {
                pref_of_currWoman = i;
            }
        }
...
printf("Instabilities: %d", not_stable);
